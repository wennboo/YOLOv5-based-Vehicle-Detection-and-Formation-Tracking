...
def doPose2(image):
    global cor
    matrix = bridge.compressed_imgmsg_to_cv2(image)
...
if __name__=="__main__":
    cor = []
    rospy.init_node("color_observer")
    anchor_cor = anchor_box()
    bridge = CvBridge()
    sub = rospy.Subscriber("/anchor_box",anchor_box,doPose1,queue_size=1)
    sub = rospy.Subscriber("/camera/image_raw/compressed",CompressedImage,doPose2,queue_size=1)
    rospy.spin()